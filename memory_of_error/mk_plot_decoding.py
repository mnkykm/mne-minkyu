...
from mk_config import res_path, plt_path, analyses, shift_onset, split_cond, split_targ
...
for analysis in analyses:   # Delete this line when using swarm!
    ...
    for cond in cond_list:
        for targ in targ_list:
            ...
            fname_image = [output_path + '/plot%i_%s_%s_%s.jpg' % (i, analysis, cond, str(targ)) for i in range(1, 5)]

            all_scores, all_diag, all_patterns = [], [], []
            for subject in subjects:
                path_subj = os.path.join(input_path, subject)
                fname_score = path_subj + '/scores_%s_%s_%s_%s.npy' % (subject, analysis, cond, str(targ))
                fname_pttrn = path_subj + '/patterns_%s_%s_%s_%s.npy' % (subject, analysis, cond, str(targ))
            ...
            axes.set_title("%s (n=%i, onset=%s, cond=%s, targ=%s)" % (analysis, n, onset_name, cond, str(targ)))
            plt.xticks(ticks)
            ...
            for idx, ax in enumerate(axes):
                if idx < n:
                    try:
                        ...
                        im = plot(np.empty_like(all_scores[0]), ax, analysis=analysis)
                else:
                    im = plot(np.empty_like(all_scores[0]), ax, analysis=analysis)
                ax.set_xticks([])
                ax.set_yticks([])
